...
def main():
    ...
    files=[ROOT/'README.md',ROOT/'bom.csv',workspace/'requirements.txt',workspace/'README.md',
           workspace/'data/reference.png', workspace/'data/xl330_m288_t.step',
           workspace/'data/xl330_m288_t_assembly.json']
    ...
    inventory=[dict(path=str(p.relative_to(workspace)),bytes=p.stat().st_size) for p in files]
    ...
    manifest.write_text(json.dumps(dict(files=inventory,file_count=len(files),total_bytes=sum(p['bytes'] for p in inventory),
                                       scope='repository files only; runtime/cache and intermediate kinematic run excluded',
                                       hardware_validated=False,full_requested_motion_release=status['full_requested_motion_release']),indent=2)+'\n')
    archive=workspace/'outputs/gorilla8_print_design.zip';archive.parent.mkdir(exist_ok=True)
    with zipfile.ZipFile(archive,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=6) as z:
        for p,row in zip(files,inventory):z.write(p,row['path'])
        z.write(manifest,'examples/gorilla8/output/delivery/file_inventory.json')
