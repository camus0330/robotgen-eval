...
def main():
    ...
    parser.add_argument('--out', type=Path, default=HERE / 'results/gorilla8')
    ...
    with (HERE / 'metrics.csv').open(newline='') as f:
    ...
    result = dict(benchmark_version=profile['benchmark_version'], profile_id=profile['profile_id'],
                  ...
                  evaluator_hashes={f.name: sha(f) for f in (HERE/'evaluate.py',HERE/'profile.json',HERE/'metrics.csv')})
    if args.review:
