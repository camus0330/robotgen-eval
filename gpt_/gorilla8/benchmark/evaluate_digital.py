...
class Digital:
    def __init__(self,root,run):
        ...
        self.profile=read(HERE/'digital_profile.json');self.motion_profile=read(HERE/'profile.json')
    ...
    def bom(self):
        ...
        good=n==8 and sum(x['name'].endswith('_case') for x in objects)==8 and (self.workspace/'xl330_m288_t/xl330_m288_t.step').is_file()
    ...
    def robustness(self):
        ...
        cases=[json.loads(s) for s in (HERE/'robustness_cases_public.jsonl').read_text().splitlines()]
        bound=r['cases_sha256']==sha(HERE/'robustness_cases_public.jsonl') and r['runner_sha256']==sha(HERE/'simulate_cases.py')
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--root',type=Path,default=HERE.parent)
    p.add_argument('--run',type=Path,default=HERE/'runs/20260920');p.add_argument('--out',type=Path,default=HERE/'results/digital')
    ...
    for ids,fn in [(['DA01','DA02','DA03'],d.exports),(['DA04'],d.bom),(['DA05','DC03'],d.geometry),(['DA06'],d.bed),
        ...
        d.check(ids,fn)
    with (HERE/'digital_metrics.csv').open() as f:catalog=list(csv.DictReader(f))
    ...
    result=dict(benchmark_version=d.profile['benchmark_version'],profile_id=d.profile['profile_id'],created_utc=datetime.now(timezone.utc).isoformat(),
                ...
                evaluator_sha256={p.name:sha(p) for p in [Path(__file__),HERE/'check_extended.py',HERE/'digital_profile.json',HERE/'digital_metrics.csv',HERE/'evaluate.py',HERE/'profile.json']})
