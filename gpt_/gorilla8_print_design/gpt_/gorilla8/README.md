...
依据上级目录的参考图设计，使用 **8 台 XL330-M288-T**：左右肩、肘、髋、膝各一台，全部为 pitch。手腕、脚踝、头和躯干没有额外电机。原图的长臂外形与猩猩面部保留为设计参考，关节配置按本次要求重新设计。
...
以下命令从仓库根目录执行，脚本按自身位置解析路径。所有源码、输入和派生产物限于 `gpt_/`，没有使用 personal skill 或其他项目的机器人资产。

```bash
UV_CACHE_DIR="$PWD/gpt_/.tools/cache" TMPDIR="$PWD/gpt_/.tools/tmp" \
  uv venv --python /usr/bin/python3 gpt_/.tools/venv
UV_CACHE_DIR="$PWD/gpt_/.tools/cache" TMPDIR="$PWD/gpt_/.tools/tmp" \
  uv pip install --python gpt_/.tools/venv/bin/python -r gpt_/gorilla8/requirements.txt

gpt_/.tools/venv/bin/python gpt_/gorilla8/src/build_robot.py
gpt_/.tools/venv/bin/python gpt_/gorilla8/src/plan_flat_palm_motion.py
gpt_/.tools/venv/bin/python gpt_/gorilla8/src/export_simulation.py
gpt_/.tools/venv/bin/python gpt_/gorilla8/src/verify_exports.py
gpt_/.tools/venv/bin/python gpt_/gorilla8/src/verify_assembly.py \
  --motion flat_palm_swing --motion flat_palm_wave --exhaustive-exact \
  --report gpt_/gorilla8/reports/flat_reference_collision.json
gpt_/.tools/venv/bin/python gpt_/gorilla8/src/make_drawings.py
```
...
```bash
gpt_/.tools/venv/bin/python gpt_/gorilla8/src/audit_strength.py \
  --simulation-result gpt_/gorilla8/output/simulation_runs/flat_palm_swing_release/result.json \
  --simulation-result gpt_/gorilla8/output/simulation_runs/flat_palm_wave_release/result.json
gpt_/.tools/venv/bin/python gpt_/gorilla8/src/finalize_release.py
gpt_/.tools/venv/bin/python gpt_/gorilla8/src/package_release.py
```
...
`src/` 为参数 CAD、动作与验证入口；`docs/` 为制造及数据说明；`output/parts/`、`assembly/`、`meshes/` 为制造几何；`output/motions/` 是参考，`simulation_runs/` 是实际仿真，`renders/`、`drawings/` 为实际几何图；`reports/` 保存回读、碰撞、质量、强度计算及诊断。

`plan_motion.py` 和 `plan_palm_swing.py` 是旧圆弧前掌方案的规划入口，不能用于当前平掌。前期圆掌失败、短行程候选和过零避让探针均是历史诊断，不能与正式交付状态混用；本地旧制造包保存在 `output/archive/before_flat_palms.zip`。最终压缩包只选取正式两项动作与正式仿真目录。
